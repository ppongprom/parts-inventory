...
/** ตัวเอง + ลูกหลานทั้งหมดของ zoneId เรียงแบบ depth-first ตาม code (แม่ก่อน แล้วไล่ลูกทีละกิ่ง) —
 *  ใช้ตอนพิมพ์ QR ของโซนแม่แล้วอยากได้ QR ลูกๆ ข้างในตามมาด้วยครบ เรียงลำดับให้แปะตามจริงได้เลย
 *  ไม่ใช่แค่รายชื่อ ids แบบสุ่มลำดับเหมือน getDescendantIds */
export function getSortedDescendants(zones, zoneId) {
  const root = zones.find((z) => z.id === zoneId);
  if (!root) return [];
  const result = [root];
