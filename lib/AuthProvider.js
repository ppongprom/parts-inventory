...
export function AuthProvider({ children }) {
  ...
  async function loadMemberships() {
    ...
    // มีแถวอยู่จริง (เคยเป็นสมาชิก) แต่ไม่มีแถวไหน active เลย = ถูกปิดใช้งานทั้งหมด
    // (ไม่ใช่ผู้ใช้ใหม่ที่ไม่เคยมีอู่ กรณีนั้น allRows จะว่างเปล่าตั้งแต่ต้น ไม่ใช่ตรงนี้)
    setIsDisabledAccount((allRows || []).length > 0 && activeRows.length === 0);
  ...
  useEffect(() => {
    ...
    // จนตัวแรกโดน cancel กลางทาง — 50ms พอกันปัญหานี้โดยไม่กระทบ UX เพราะผู้ใช้ไม่มีทางรู้สึก
    // หน่วงระดับนี้ได้
    const debounceTimer = setTimeout(() => {
      (async () => {
        ...
        heartbeatInterval = setInterval(async () => {
          // เดิม eviction แค่ลบแถวใน user_sessions โดยไม่เคยตัดสิทธิ์เครื่องที่ถูกเขี่ยจริง (JWT
          // เดิมยังใช้ได้ต่อจนหมดอายุเอง) ตอนนี้ heartbeat เช็คว่าแถวของตัวเองยังอยู่ไหมทุกครั้ง
          // ถ้าหายไป (ถูก evict จากเครื่องอื่นที่ login ชนกัน) ให้ signOut ทันที ไม่ปล่อยให้ใช้งาน
          // ต่อเงียบๆ
